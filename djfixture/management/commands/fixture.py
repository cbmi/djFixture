...
class Command(BaseCommand):
    ...
    def get_subcommand(self, name):
        try:
            module = import_module('djfixture.management.subcommands.{0}'.format(name))
            return module.Command()
        except KeyError:
            raise CommandError('Unknown subcommand: fixture {0}'.format(name))
    ...
    def handle(self, *args, **options):
        if not args or args[0] not in self.commands:
            return self.print_help('./manage.py', 'fixture')
