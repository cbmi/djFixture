...
def csv2fixture(self,file,reader,jsonModels,pName,fout):
    ...
    for form in open(jsonModels,'r'):
        ...
        for line in reader:
            ...
            if numRepeats:
                ...
                if new_form_name:
                    foreign_forms_list = find_related_forms(self,new_form_name,form_dict);
                    new_form_name = '';
                else:
                    foreign_forms_list = find_related_forms(self,form_name,form_dict);
                full_form_list = foreign_forms_list[:];
                full_form_list.append(base_form_name);
                full_form_list = full_form_list[::-1];

                primary_key_counter = generate_repeating_fixtures(self,line,form,full_form_list,fixtures,pk_num,pk_num_list,primary_key_counter);
            else:
                ...
                for field in form['fields']:
                    ...
                    """
                    if field['choices']:
                        field_names = get_field_names(self,field,form_dict,field['field name']);
                        checked_line = '';
                        answered = '';
    ...
    printFixtures(self,fixtures,pk_num_list,fout);
    ...
    for i in range(num_repeats_all):
        ...
        for field in form['fields']:
            ...
            if len(form_list[2:]) == 0:
                ...
                base_field_name = get_field_name(self,field,form_list[2:],current_repeat_list).lower();
            if field['choices']:
                field_names = get_field_names(self,field,form_list[2:],base_field_name);
                checked_line = '';
                answered = '';
                for name in field_names:
                    try:
        ...
        update_current_repeats(self,num_repeats_list[::-1],current_repeat_list,cur_ind)
...
def update_current_repeats(self,form_list,current_repeats_list,cur_index):
    ...
    if int(current_repeats_list[cur_index]) >= int(form_list[cur_index]):
        ...
        if cur_index - 1 >= 0:
            cur_index -= 1;
            update_current_repeats(self,form_list,current_repeats_list,cur_index);
    else:
        current_repeats_list[cur_index] += 1;
